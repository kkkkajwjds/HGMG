...
class node_embedding(nn.Module):
    ...
    def forward(self, adj_t, adj_ot, X_t, X_ot):
        ...
        combined = self.alpha * agg_ot + (1 - self.alpha) * agg_t
        X1 = self.leaky_relu(self.W1(combined))
        X1 = F.dropout(X1, self.dropout, training=self.training)

        # 第二层只使用同质邻接矩阵
        if adj_t_norm.is_sparse:
            agg_t2 = torch.sparse.mm(adj_t_norm, X1)
        else:
            agg_t2 = adj_t_norm @ X1
        X2 = self.leaky_relu(self.W2(agg_t2))
